...
Deno.serve(async request => {
  ...
  try {
    ...
    if(request.method==='POST'&&path==='/games/roulette/bets') {
      ...
      if(error) { const code=['BETTING_CLOSED','BET_ALREADY_PLACED_OTHER_ROOM','BET_ALREADY_PLACED','INSUFFICIENT_BALANCE','BET_POSITION_LIMIT_EXCEEDED','BET_LIMIT_EXCEEDED','BET_BELOW_MINIMUM','INVALID_AMOUNT','INVALID_BETS','INVALID_BET','INVALID_BET_VALUE'].find(c=>error.message.includes(c))||'BET_FAILED'; return fail(code,code==='INSUFFICIENT_BALANCE'?'게임머니가 부족합니다.':code==='BETTING_CLOSED'?'현재 라운드의 베팅이 마감되었습니다.':code==='BET_ALREADY_PLACED_OTHER_ROOM'?'같은 라운드 구간에 다른 테이블에서 이미 베팅했습니다.':code==='BET_ALREADY_PLACED'?'이 라운드에는 이미 베팅을 완료했습니다.':code==='BET_POSITION_LIMIT_EXCEEDED'?'한 라운드에는 최대 20곳까지 베팅할 수 있습니다.':code==='BET_LIMIT_EXCEEDED'?'이 테이블의 최대 베팅 한도를 초과했습니다.':code==='BET_BELOW_MINIMUM'?'이 테이블의 최소 베팅 금액보다 작습니다.':'베팅 요청이 올바르지 않습니다.',code==='INSUFFICIENT_BALANCE'||code==='BET_ALREADY_PLACED'||code==='BET_ALREADY_PLACED_OTHER_ROOM'?409:400); }
