alter table public.roulette_rooms
  add column if not exists min_bet bigint not null default 1,
  add column if not exists max_bet bigint not null default 20,
  add column if not exists chip_values bigint[] not null default array[1,2,5,10,20]::bigint[];

update public.roulette_rooms set min_bet=1,max_bet=20,chip_values=array[1,2,5,10,20]::bigint[] where id='main';
update public.roulette_rooms set min_bet=10,max_bet=200,chip_values=array[10,20,50,100,200]::bigint[] where id='vip-1';
update public.roulette_rooms set min_bet=20,max_bet=500,chip_values=array[20,50,100,200,500]::bigint[] where id='vip-2';
