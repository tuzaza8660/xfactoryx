create table if not exists public.roulette_rooms (
  ...
  max_players smallint not null default 50 check (max_players between 1 and 500),
  is_active boolean not null default true,
  created_at timestamptz not null default now()
);

insert into public.roulette_rooms(id,display_name) values('main','Main Table') on conflict do nothing;

alter table public.roulette_rounds add column if not exists room_id text not null default 'main' references public.roulette_rooms(id);
alter table public.roulette_rounds drop constraint if exists roulette_rounds_round_key_key;
create unique index if not exists roulette_rounds_room_round_key_idx on public.roulette_rounds(room_id,round_key);
create index if not exists roulette_rounds_room_starts_idx on public.roulette_rounds(room_id,starts_at desc);

alter table public.roulette_rooms enable row level security;
create policy "read active roulette rooms" on public.roulette_rooms for select to authenticated using(is_active);
...
begin
  ...
  if jsonb_typeof(p_bets)<>'array' or jsonb_array_length(p_bets)<1 or jsonb_array_length(p_bets)>35 then raise exception 'BET_POSITION_LIMIT_EXCEEDED'; end if;
  for v_item in select value from jsonb_array_elements(p_bets) loop
    ...
    if v_amount<=0 or v_amount>100000 then raise exception 'INVALID_AMOUNT'; end if;
  ...
  if v_total>500000 then raise exception 'BET_LIMIT_EXCEEDED'; end if;
