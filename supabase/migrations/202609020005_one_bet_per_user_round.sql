alter table public.roulette_bet_slips
  add column if not exists single_submission boolean not null default false;

alter table public.roulette_bet_slips
  alter column single_submission set default true;

create unique index if not exists roulette_bet_slips_one_submission_idx
  on public.roulette_bet_slips(round_id,user_id)
  where single_submission;
...
begin
  ...
  select * into v_slip from public.roulette_bet_slips where round_id=p_round_id and user_id=p_user_id order by created_at limit 1;
  if found then raise exception 'BET_ALREADY_PLACED'; end if;
